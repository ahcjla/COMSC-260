;Programmer name: Aurelia Kisanaga
;
...
StrNCpyAsm PROC uses eax esi edi ecx    ,   ;save registers used
	...
	mov     edi, str1Add        ;move address of string 1 to edi
	mov     esi, str2Add        ;move address of string 2 to esi
	mov     ecx, count          ;copy value of count to ecx
    ...
    push    str2Add             ;push address of string 2 to StrLenAsm
    call    StrLenAsm           ;get length of string 2
    add     eax, 1              ;make length of string to be zero terminated at the end

    cmp     eax, ecx            ;compare if string is longer than count value (ecx)
    jae     greater             ;if string length greater, jump to greater
    mov     ecx, eax            ;copy length of string stored in eax to ecx

greater:
    cld                         ;clears directional flag
    rep     movsb               ;fetches byte at address esi to edi, increments it and repeat ecx times
...
StrCatAsm PROC  uses eax edi ecx esi  ,  ;save registers used
	...
	mov     edi, str1Add            ;copy address of string 1 to edi
	mov     esi, str2Add            ;copy address of string 2 to esi
    ...
    mov     ecx, MAX_LEN            ;copy maximum length (constant) to ecx
    mov     eax, 0                  ;stores 0 in eax

    cld                             ;clears directional flag, edi increments
    repne   scasb                   ;searches for zero terminated string

    sub     edi, 1                  ;moves edi by 1 pointing to the left so it ends at last char and not zero terminated
    push    esi                     ;push address of string 2
...
;
; Note: there should only be one loop in StrReverse
;
...
StrReverse proc uses ecx eax edi esi,   ;save registers used
                   str1Add:dword,       ;address of string1
                   str2Add:dword        ;address of string2
    ...
    push    str2Add             ;push address of string 2 argument to StrLenAsm
    call    StrLenAsm           ;get length of string 2

    mov     ecx, eax            ;moves length of string in eax to ecx

    add     esi, ecx            ;point address to the end of the array 
    sub     esi, 1              ;moves esi by 1 pointing to the left so it ends at last char and not zero terminated

loopTop:
    std                         ;set direction flag so esi decrements 
    lodsb                       ;byte of reg esi loaded into AL. (Al = byte ptr [esi])
    cld                         ;clears direction flag so edi increments
    stosb                       ;copy byte of reg edi from register AL (byte ptr [edi] = AL)
    loop    loopTop             ;repeat ecx times

    mov     byte ptr[edi], 0    ;terminate copied string to null
...
;See the program 7 specifications document (pdf file)
