...
; stack floating point can be confusing
;
; Short description of what program does:
; Print the principal money, interest rate, compound year/month/day and its length provided in the array
; and calculate its compound interest and print it
;
; *************************************************************
...
.data
    ...
    heading         byte   "Program 8 by Aurelia Kisanaga",LF,LF,0
...
Main PROC
    ...
    ;fidiv operand2: st(0) = st(0) / operand2 (float/ integer)
    ...
    call    TSeparatorOn                    ;turn thousand seperator
    mov     ecx, 2                          ;set ecx = 2
    call    SetFloatPlaces                  ;set floats decimal places to 2
    mov     ecx, 13                         ;set ecx = 13
    call    SetFieldWidth                   ;set field width to 13

    mov     ebx, 0                          ;initializes counter ebx
    mov     esi, 0                          ;initializes counter esi

    mPrtStr heading                         ;print heading
    mPrtStr heading2                        ;print heading2

loopTop:
    cmp     esi, TOTAL_SIZE                 ;compare if counter esi equals to size of array
    je      done                            ;if equal jump to done

    ;output principal value
    fld     principal[ebx]                  ;push principal value to float point stack
    call    PrefixOn                        ;set prefix $ on
    call    DspFloatP                       ;print st(0) and pop from stack

    ;output rate percentage
    fld     rate[ebx]                       ;push rate value to float point stack
    fmul    hundred                         ;multiply 100 to turn into percent
    call    PrefixOff                       ;turn prefix $ off
    call    DspFloatP                       ;print st(0) and pop from stack
    mPrtChar   '%'                          ;print percentage % sign

    ;comparison
    cmp     compoundedYlyMlyDly[esi], 1     ;compare compoundedYlyMlyDly to 1
    je      Y                               ;if true, jump to Y
    cmp     compoundedYlyMlyDly[esi], 12    ;else compare compoundedYlyMlyDly to 12
    je      M                               ;if true, jump to M

    mPrtStr daily                           ;else it is daily, print D for daily
    jmp     equation                        ;jump to compound interest equation 

Y:
    mPrtStr yearly                          ;print Y for yearly
    jmp     equation                        ;jump to compound interest equation

M:
    mPrtStr monthly                         ;print M for monthly

equation:
    mPrtDec period[esi]                     ;print the period

    ;A = P (1 + r/n)^nt
    ;(r/n)
    fld     rate[ebx]                       ;push rate value to floating point stack
    fidiv   compoundedYlyMlyDly[esi]        ;st(0) = st(0) / compoundedYlyMlyDly

    ;(1 + r/n)
    fadd    increment                       ;st(0) = st(0) + increment 

    ;(n*t)
    mov     eax, compoundedYlyMlyDly[esi]   ;move compoundedYlyMlyDly to eax
    mul     period[esi]                     ;eax = eax * period        
    mov     ecx, eax                        ;move eax to ecx

    ;(1 + r/n)^nt
    call    Power                           ;call power func: st(0) = st(0) ^ ecx

    ;P * (1 + r/n)^nt
    fmul    principal[ebx]                  ;st(0) = st(0) * principal  


    call    PrefixOn                        ;set prefix $ on
    call    DspFloatP                       ;print st(0) and pop from stack

    add     ebx, 8                          ;increment ebx by 8
    add     esi, 4                          ;increment esi by 4

    mPrtChar LF                             ;print new line character
...
;Note: there is no exit message. No "Hit any key to exit".
